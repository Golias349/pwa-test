...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&drawer.classList.contains('show'))$('#btnCloseDrawer').click();});
$$('.drawer-link').forEach(b=>b.addEventListener('click',()=>{const open=b.dataset.open;if(open){Object.values(sections).forEach(s=>s.classList.add('hidden'));sections[open].classList.remove('hidden');}$('#btnCloseDrawer').click();if(open==='resumo'){preencherFiltrosResumo();renderResumo();}}));
...
/* ===== Exportações ===== */
function exportCSV(){const m=parseInt($('#mesResumo').value),a=parseInt($('#anoResumo').value);const ag=aggregateResumo(m,a);const rows=[['Talhão','Insumo','Kg aplicados','Gasto (R$)','Mês/Ano']].concat(ag.rows.map(r=>[r.talhao,r.insumo,r.kg.toFixed(2),r.gasto.toFixed(2),`${String(m).padStart(2,'0')}/${a}`]));rows.push(['TOTAL','','',ag.totG.toFixed(2),`${String(m).padStart(2,'0')}/${a}`]);const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(';')).join('\n');const aEl=document.createElement('a');aEl.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));aEl.download=`resumo-${a}-${String(m).padStart(2,'0')}.csv`;aEl.click();}
$('#btnExportCSV').onclick=exportCSV;
function exportPDF(){const m=parseInt($('#mesResumo').value),a=parseInt($('#anoResumo').value);const ag=aggregateResumo(m,a);const w=window.open('','_blank');const tbody=ag.rows.map(r=>`<tr><td>${r.talhao}</td><td>${r.insumo}</td><td>${r.kg.toFixed(2)}</td><td>${r.gasto.toFixed(2)}</td><td>${String(m).padStart(2,'0')}/${a}</td></tr>`).join('');const html=`<!doctype html><html><head><meta charset="utf-8"><title>Resumo Mensal</title><style>body{font-family:Arial,Helvetica,sans-serif;padding:20px}h2{margin:0 0 10px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #999;padding:6px;text-align:left}thead th{background:#eee}</style></head><body><h2>Resumo Mensal ${String(m).padStart(2,'0')}/${a}</h2><table><thead><tr><th>Talhão</th><th>Insumo</th><th>Kg aplicados</th><th>Gasto (R$)</th><th>Mês/Ano</th></tr></thead><tbody>${tbody}</tbody></table></body></html>`;w.document.write(html);w.document.close();w.focus();w.print();setTimeout(()=>w.close(),600);}
$('#btnExportPDF').onclick=exportPDF;

/* ===== Backup Local & Drive (somente na aba Config) ===== */
$('#btnExportJSON')?.addEventListener('click',()=>{const payload={talhoes:db.get(K.TALHOES,[]),estoque:db.get(K.ESTOQUE,[]),registros:db.get(K.REG,[])};const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));a.download='grao-digital-backup.json';a.click();});
...
function initGoogle(){const s=document.createElement('script');s.src='https://accounts.google.com/gsi/client';s.onload=()=>{google.accounts.oauth2.initTokenClient({client_id:CLIENT_ID,scope:SCOPES,callback:(token)=>{accessToken=token.access_token;alert('Conectado ao Google.');}}).requestAccessToken();};document.body.appendChild(s);}$('#btnGoogleConectar')?.addEventListener('click',initGoogle);
